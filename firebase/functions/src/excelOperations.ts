...
export const exportToExcel = runWith({
  ...
  .https.onCall(async (data, context) => {
    ...
    const churches = (
      await firestore().collection("Churches").get()
    ).docs.reduce((map, obj) => {
      map[obj.id] = obj.data()?.Name ? obj.data().Name : "(غير معروف)";
      return map;
    }, {} as Record<string, string>);
    const cfathers = (
      await firestore().collection("Fathers").get()
    ).docs.reduce((map, obj) => {
      map[obj.id] = obj.data()?.Name ? obj.data().Name : "(غير معروف)";
      return map;
    }, {} as Record<string, string>);
    ...
    if (data?.onlyService) {
      ...
      services = (
        currentUser.customClaims.superAccess
          ? (await firestore().collection("Services").orderBy("Name").get())
              .docs
          : await Promise.all(
              (
    ...
    const persons = (
                ...
                async (chunk) =>
                  await firestore()
                    ...
                    .where(
                      ...
                      "array-contains-any",
                      chunk.map((a) =>
                        firestore().collection("Services").doc(a)
                      )
                    )
                    .orderBy("Name")
                    .get()
      ...
      rslt["ClassId"] = (
        p.data()["ClassId"] as firestore.DocumentReference
      )?.id;
...
export const importFromExcel = runWith({
  ...
  .https.onCall(async (data, context) => {
    ...
    if (
      ...
      !currentUser.customClaims?.write
    ) {
      throw new https.HttpsError(
        "permission-denied",
    ...
    const persons = utils
      ...
      .reduce<Array<Record<string, string | number | Record<string, any>>>>(
        (arry, currentPerson) => {
          ...
          rslt["FatherPhone"] =
            person["Father Phone Number"]?.toString() ?? null;
          rslt["MotherPhone"] =
            person["Mother Phone Number"]?.toString() ?? null;
          rslt["Phones"] = {};
          rslt["Services"] =
            person["Services"] !== null &&
            person["Services"] !== undefined &&
            (JSON.parse(person["Services"] as string) as Array<string>).length >
              0
